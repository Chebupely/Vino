...
def find_vino(numbers_of_vino, delta_sample):
    # Путь до данных
    data_path = r'backend/AgroCodeHack_VINEYARDS'
    # Путь до сохранения картиинки
    res_pic_path = r'backend/Resulting_pic'
    # читаем json
    ...
    for data_sample in information:
        ...
        for coord in result:
            ...
            ax.add_patch(rect)
        plt.axis('off')
        plt.savefig(os.path.join(r'main/static/main/img/', 'result.JPEG'), format='jpeg', dpi=1000, pad_inches=0,
                    bbox_inches='tight')
        plt.axis('off')
    return 0
